...
fileName = basedir + '//testCases//apiTestData//global//centerapi//' + os.path.basename(__file__).split('.')[0] + '.yml'
readfile = ReadFile(fileName)
cases, parameters = readfile.get_businessTests_data()


@allure.feature('get platforms configs')
class TestCheckEmailExisted:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.globaltest
    @pytest.mark.kyle
    @pytest.mark.centerapi
    @pytest.mark.smoke
    def test_get_platform_configs(self, case, http, expect, login):
        login_info = login
        ...
        get_response = test_api_request(login_info['centerUrl'] + http['path'].replace('$email$', str(site_email)), http['method'], header,
                                        None)
