...
@allure.feature('put child partner by id')
class TestPutChildPartnerById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.childpartnerapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_put_child_partner_by_id(self, case, http, expect, partnerLogin, create_and_delete_child_partner):
        login_info = partnerLogin
        initial_oauth_partner = create_and_delete_child_partner
        ...
        get_response = test_api_request(login_info['partnerUrl'] + http['path'].replace('$id$', str(initial_oauth_partner['id'])),http['method'], login_info['partnerCommonHeader'], http['body'])
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_put_child_partner_by_id_auth_token(self, case, http, expect, partnerLogin, create_and_delete_child_partner):
        login_info = partnerLogin
        initial_oauth_partner = create_and_delete_child_partner
        partner_id = ManageGlobalData().get_partner_id()
        body = change_to_random_string(http['body'])
        # pdb.set_trace()
        # body['partnerUsers'][0]['confirmEmail'] = str(body['partnerUsers'][0]['email'])
        get_response = test_api_request(login_info['partnerUrl'] + http['path'].replace('$id$', str(initial_oauth_partner['id'])), http['method'],login_info['partner_oauth_header'], http['body'])
        commonResponseAssert(get_response, expect)
