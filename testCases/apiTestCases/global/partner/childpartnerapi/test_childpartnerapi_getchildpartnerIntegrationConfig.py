...
@allure.feature('get child partner Integration Config')
class TestGetIntegrationConfigOfChildPartnerById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.childpartnerapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_integration_config_of_child_partner_by_id(self, case, http, expect, partnerLogin):
        login_info = partnerLogin
        partner_id = ManageGlobalData().get_partner_id()
        get_response = test_api_request(login_info['partnerUrl'] + http['path'].replace('$id$', str(partner_id)),http['method'], login_info['partnerCommonHeader'], None)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_integration_config_of_child_partner_by_id_auth_token(self, case, http, expect, partnerLogin):
        login_info = partnerLogin
        partner_id = ManageGlobalData().get_partner_id()
        get_response = test_api_request(login_info['partnerUrl'] + http['path'].replace('$id$', str(partner_id)),http['method'], login_info['partner_oauth_header'], None)
        commonResponseAssert(get_response, expect)
