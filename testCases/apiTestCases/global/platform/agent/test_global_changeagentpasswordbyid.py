...
@allure.feature('change agent password by id')
class TestChangeAgentPasswordById:
    ...
    def test_change_agent_password(self, case, http, expect, login, initial_data):
        login_info = login
        initial_agent = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_agent['id'])
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_change_agent_password_api_key(self, case, http, expect, login, initial_data):
        login_info = login
        initial_agent = initial_data
        site_id = ManageGlobalData().get_site_id()
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_change_agent_password_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_agent = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_agent['id'])
