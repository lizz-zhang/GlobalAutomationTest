...
@allure.feature('unlock agent by id')
class TestUnlockAgentById:
    ...
    def test_unlock_agent_by_id(self, case, http, expect, login, initial_data):
        login_info = login
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_unlock_agent_by_id_apikey(self, case, http, expect, login, initial_data):
        login_info = login
        initial_agent = initial_data
        site_id = ManageGlobalData().get_site_id()
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_unlock_agent_by_id_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_agent = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(
            login_info['dashUrl'] + http['path'].replace('$id$', initial_agent['id']).replace('$siteId$', str(site_id)),
            http['method'], login_info['oauth_header'],
            None)
        commonResponseAssert(get_response, expect)