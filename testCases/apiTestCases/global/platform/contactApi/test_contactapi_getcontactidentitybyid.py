...
@allure.feature('get a contact identity by id')
class TestGetContactsIdentityByIdNew:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.parametrize('initial_data', ['contacts_new'], indirect=True)
    @pytest.mark.contactapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_contactidentity_new_by_id(self, case, http, expect, login, initial_data):
        login_info = login
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_contactidentity_by_id_api_key(self, case, http, expect, login, initial_data):
        login_info = login
        initial_contact = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_contact['contactIdentities'][0]['id']).replace('$siteId$', str(site_id)), http['method'],
                                        None)
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_contactidentity_by_id_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_contact = initial_data
        site_id = ManageGlobalData().get_site_id()
