...
@allure.feature('put a contact identity by id')
class TestPutContactIdentityById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.parametrize('initial_data', ['contacts_new'], indirect=True)
    @pytest.mark.contactapi
    @pytest.mark.oden
    @pytest.mark.smoke
    def test_put_contact_identity_by_id(self, case, http, expect, login, initial_data):
        login_info = login
        initial_contact = initial_data
        site_id = ManageGlobalData().get_site_id()
        for i in initial_contact['contactIdentities']:
            if i['contactIdentityType'] == 'SMS':
                sms_id = i['id']
                break

        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', sms_id)
                                        .replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'], http['body'])
    ...
    @pytest.mark.oden
    @pytest.mark.smoke
    def test_put_contact_identity_by_id_api_key(self, case, http, expect, login, initial_data):
        login_info = login
        initial_contact = initial_data
        site_id = ManageGlobalData().get_site_id()
        for i in initial_contact['contactIdentities']:
        ...
        get_response = test_api_basic_auth_request(login_info['dashUrl'] + http['path'].replace('$id$', sms_id)
                                        .replace('$siteId$', str(site_id)), http['method'], http['body'])
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.oden
    @pytest.mark.smoke
    def test_put_contact_identity_by_id_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_contact = initial_data
        site_id = ManageGlobalData().get_site_id()
        for i in initial_contact['contactIdentities']:
        ...
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', sms_id)
                                        .replace('$siteId$', str(site_id)), http['method'], login_info['oauth_header'], http['body'])
        commonResponseAssert(get_response, expect)
