...
@allure.feature('get contact filed by id')
class TestContactFields:
    ...
    @pytest.mark.oden
    @pytest.mark.smoke
    def test_get_contact_field_by_id(self, case, http, expect, login, initial_data):
        login_info = login
        initial_field = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_field['id']).replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'],
                                        None)
    ...
    @pytest.mark.oden
    @pytest.mark.smoke
    def test_get_contact_field_by_id_apikey(self, case, http, expect, login, initial_data):
        login_info = login
        initial_field = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_field['id']).replace('$siteId$', str(site_id)), http['method'],
                                        None)
    ...
    def test_get_contact_field_by_id_oauth_token(self, case, http, expect, login, initial_data):
        ...
        initial_field = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_field['id']).replace('$siteId$', str(site_id)), http['method'], login_info['oauth_header'],
                                        None)
