...
class TestRefresh:
    ...
    def test_refresh(self, case, http, expect, login):
        login_info = login
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_refresh_api_key(self, case, http, expect, login):
        login_info = login
        env_dash = login_info['dashUrl']
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(env_dash + http['path'].replace('$siteId$', str(site_id)), http['method'], None)
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_agents_oauth_token(self, case, http, expect, login):
        login_info = login
        env_dash = login_info['dashUrl']
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(env_dash + http['path'].replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'], None)
        commonResponseAssert(get_response, expect)
