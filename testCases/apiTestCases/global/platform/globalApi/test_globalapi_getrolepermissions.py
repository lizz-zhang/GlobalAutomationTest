...
class TestGetRolePermissions:
    ...
    def test_get_role_permissions(self, case, http, expect, login, initial_data):
        login_info = login
        initial_role_id = initial_data['id']
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$roleId$', str(initial_role_id)), http['method'], login_info['commonHeader'], None)
        commonResponseAssert(get_response, expect)

        """
        This interface error when use apiKey authentication. It is a bug
        get_response = test_api_basic_auth_request(login_info['dashUrl'] + http['path'].replace('$roleId$', str(initial_role_id)), http['method'], None)
        commonResponseAssert(get_response, expect)
        """

        get_response_oauth_token = test_api_request(login_info['dashUrl'] + http['path'].replace('$roleId$', str(initial_role_id)), http['method'], login_info['oauth_header'], None)
