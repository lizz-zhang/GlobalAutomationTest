...
@allure.feature('put a skill by id')
class TestPutSkillById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.parametrize('initial_data', ['skills'], indirect=True)
    @pytest.mark.globaltest
    @pytest.mark.oden
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_skill_by_id(self, case, http, expect, login, initial_data):
        login_info = login
        initial_skill = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_skill['id'])
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_skill_by_id_api_key(self, case, http, expect, login, initial_data):
        login_info = login
        initial_skill = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(
            login_info['dashUrl'] + http['path'].replace('$id$', initial_skill['id']).replace('$siteId$', str(site_id)),
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_skill_by_id_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_skill = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_skill['id'])
