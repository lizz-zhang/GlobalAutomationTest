...
class TestPostPublicCannedMessage:
    ...
    def test_post_public_cannedmessage(self, case, http, expect, login):
        login_info = login
        category_Id = get_public_canned_message_rootcategory(login)
        http['body']['categoryId'] = http['body']['categoryId'].replace('$categoryId$', str(category_Id))
    ...
    def test_post__public_cannedmessage_illegal(self, case, http, expect, login):
        login_info = login
        category_Id = get_public_canned_message_rootcategory(login)
        if ifItemInKeysAndValueNotNone('categoryId', http['body']):
            http['body']['categoryId'] = http['body']['categoryId'].replace('$categoryId$', str(category_Id))
        get_response = test_api_request(login_info['dashUrl'] + http['path'], http['method'], login_info['commonHeader'], http['body'])
        commonResponseAssert(get_response, expect)
