...
@allure.feature('put a oauth_platform by id')
class TestPutOauthPlatformById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.globalapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_put_oauth_platform_by_id(self, case, http, expect, login, create_and_delete_oauth_platform):
        login_info = login
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_put_oauth_platform_by_id_api_key(self, case, http, expect, login, create_and_delete_oauth_platform):
        login_info = login
        initial_oauth_platform = create_and_delete_oauth_platform
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(
            login_info['dashUrl'] + http['path'].replace('$id$', initial_oauth_platform['id']).replace('$siteId$',str(site_id)),http['method'], http['body'])
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_put_oauth_platform_by_id_oauth_token(self, case, http, expect, login, create_and_delete_oauth_platform):
        login_info = login
        initial_oauth_platform = create_and_delete_oauth_platform
        site_id = ManageGlobalData().get_site_id()
