...
@allure.feature('put restrictedWordsConfig')
class TestrestrictedWordsConfig:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.globaltest
    @pytest.mark.lizz
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_restrictedWordsConfig(self, case, http, expect, login):
        login_info = login
        ...
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['commonHeader'], http['body'])
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_restrictedWordsConfig_api_key(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(
            login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)), http['method'], http['body'])
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_restrictedWordsConfig_oauth_token(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['oauth_header'], http['body'])
        commonResponseAssert(get_response, expect)
