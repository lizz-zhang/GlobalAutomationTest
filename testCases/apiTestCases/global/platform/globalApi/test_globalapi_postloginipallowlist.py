...
class TestPostLoginIpAllowlist:
    @pytest.mark.globalapi
    @pytest.mark.loginipallowlist
    @pytest.mark.bella
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    def test_post_login_ip_allowlist(self, case, http, expect, login):
        login_info = login
    ...
    def test_post_login_ip_allowlist_oauth_token(self, case, http, expect, login):
        login_info = login
    ...
    @pytest.mark.loginipallowlist
    @pytest.mark.bella
    def test_post_login_ip_allowlist_illegal(self, case, http, expect, login):
        login_info = login
        get_response = test_api_request(login_info['dashUrl'] + http['path'], http['method'], login_info['commonHeader'], http['body'])
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.loginipallowlist
    @pytest.mark.bella
    def test_post_login_ip_allowlist_illegal_oauth_token(self, case, http, expect, login):
        login_info = login
        get_response_oauth_token = test_api_request(login_info['dashUrl'] + http['path'], http['method'], login_info['oauth_header'], http['body'])
        commonResponseAssert(get_response_oauth_token, expect)