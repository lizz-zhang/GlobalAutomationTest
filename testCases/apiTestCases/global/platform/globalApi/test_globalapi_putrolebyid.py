...
@allure.feature('put a role by id')
class TestPutRoleById:
    ...
    def test_put_role_by_id(self, case, http, expect, login, initial_data):
        login_info = login
        initial_role = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_role['id'])
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_role_by_id_api_key(self, case, http, expect, login, initial_data):
        login_info = login
        initial_role = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(
            login_info['dashUrl'] + http['path'].replace('$id$', initial_role['id']).replace('$siteId$', str(site_id)),
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_put_role_by_id_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_role = initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_role['id'])
