...
@allure.feature('get restrictedWordsConfig')
class TestGetrestrictedWordsConfig:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.globaltest
    @pytest.mark.lizz
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_get_restrictedWordsConfig(self, case, http, expect, login, create_and_delete_agent):
        login_info = login
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_get_restrictedWordsConfig_api_key(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['commonHeader'], None)
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_get_restrictedWordsConfig_oauth_token(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['commonHeader'], None)
