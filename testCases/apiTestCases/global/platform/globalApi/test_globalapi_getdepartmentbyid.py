...
@allure.feature('get a department by id')
class TestGetDepartmentById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.globalapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_department_by_id(self, case, http, expect, login, create_and_delete_department):
        login_info = login
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_department_by_id_api_key(self, case, http, expect, login, create_and_delete_department):
        login_info = login
        initial_department = create_and_delete_department
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_department['id']).replace('$siteId$', str(site_id)), http['method'],
                                        None)
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_department_by_id_oauth_token(self, case, http, expect, login, create_and_delete_department):
        login_info = login
        initial_department = create_and_delete_department
        site_id = ManageGlobalData().get_site_id()
