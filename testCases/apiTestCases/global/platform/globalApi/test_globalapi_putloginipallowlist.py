...
class TestPutLoginIpAllowlist:
    @pytest.mark.globalapi
    @pytest.mark.loginipallowlist
    @pytest.mark.bella
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.parametrize('initial_data', ['loginipallowlist'] , indirect=True)
    def test_put_login_ip_allowlist(self, case, http, expect, login, initial_data):
        login_info = login
        initial_login_ip_allowlist_id = initial_data['id']
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', str(initial_login_ip_allowlist_id)), http['method'], login_info['commonHeader'], http['body'])
        commonResponseAssert(get_response, expect)

        if ifItemInKeysAndValueNotNone('aftercaseaction', expect):
            expect['aftercasepath'] = expect['aftercasepath'].replace('$id$', str(initial_login_ip_allowlist_id))
    ...
    @pytest.mark.parametrize('initial_data', ['loginipallowlist'] , indirect=True)
    def test_put_login_ip_allowlist_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_login_ip_allowlist_id = initial_data['id']
        get_response_oauth_token = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', str(initial_login_ip_allowlist_id)), http['method'], login_info['oauth_header'], http['body'])
