...
@allure.feature('post disable restrictedwordsconfig')
class TestPostdisablerestrictedwordsconfig:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.globaltest
    @pytest.mark.lizz
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_post_disable_restrictedwordsconfig(self, case, http, expect, login):
        login_info = login
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_post_disable_restrictedwordsconfig_api_key(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
    ...
    @pytest.mark.smoke
    @pytest.mark.globalapi
    def test_post_disable_restrictedwordsconfig_oauth_token(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['commonHeader'], None)
