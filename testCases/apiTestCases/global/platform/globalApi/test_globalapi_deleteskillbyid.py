...
@allure.feature('delete a skill by id')
class TestDeleteSkillById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.parametrize('function_initial_data', ['skills'], indirect=True)
    ...
    def test_delete_role_by_id(self, case, http, expect, login, function_initial_data):
        login_info = login
        initial_skill = function_initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_skill['id']).replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'],
                                        None)
    ...
    def test_delete_role_by_id_apikey(self, case, http, expect, login, function_initial_data):
        ...
        initial_skill = function_initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_skill['id']).replace('$siteId$', str(site_id)), http['method'],
                                        None)
    ...
    def test_delete_role_by_id_oauth_token(self, case, http, expect, login, function_initial_data):
        ...
        initial_skill = function_initial_data
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', initial_skill['id']).replace('$siteId$', str(site_id)), http['method'], login_info['oauth_header'],
                                        None)
