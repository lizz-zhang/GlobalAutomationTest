...
class TestPutPublicCannedMessage:
    ...
    def test_put_public_cannedmessage(self, case, http, expect, login, initial_data):
        login_info = login
        initial_public_canned_message_id = initial_data['id']
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', str(initial_public_canned_message_id)), http['method'], login_info['commonHeader'], http['body'])
        commonResponseAssert(get_response, expect)

        if ifItemInKeysAndValueNotNone('aftercaseaction', expect):
            expect['aftercasepath'] = expect['aftercasepath'].replace('$id$', str(initial_public_canned_message_id))
    ...
    def test_put__public_cannedmessage_category_oauth_token(self, case, http, expect, login, initial_data):
        login_info = login
        initial_public_canned_message_id = initial_data['id']
        get_response_oauth_token = test_api_request(login_info['dashUrl'] + http['path'].replace('$id$', str(initial_public_canned_message_id)), http['method'], login_info['oauth_header'], http['body'])
