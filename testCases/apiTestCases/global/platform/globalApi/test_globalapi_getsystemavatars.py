...
@allure.feature('get systemavatars')
class Testsystemavatars:
    # request_url, request_type, request_header, request_body
    @pytest.mark.globalapi
    @pytest.mark.lizz
    @pytest.mark.smoke
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    def test_systemavatars(self, case, http, expect, login):
        login_info = login
    ...
    @pytest.mark.lizz
    @pytest.mark.smoke
    def test_systemavatars_api_key(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['commonHeader'], None)
    ...
    @pytest.mark.lizz
    @pytest.mark.smoke
    def test_systemavatars_oauth_token(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['dashUrl'] + http['path'].replace('$siteId$', str(site_id)),
                                        http['method'], login_info['commonHeader'], None)
