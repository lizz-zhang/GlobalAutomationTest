...
@allure.feature('delete a contact by id')
class TestDeleteContactById:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.contactapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_delete_contact_by_id(self, case, http, expect, login, every_function_create_contact):
        login_info = login
        initial_contact = every_function_create_contact
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['apiUrl'] + http['path'].replace('$id$', initial_contact['id']).replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'],
                                        None)
    ...
    @pytest.mark.smoke
    @pytest.mark.apikey
    def test_delete_contact_by_id_apikey(self, case, http, expect, login, every_function_create_contact):
        login_info = login
        initial_contact = every_function_create_contact
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(login_info['apiUrl'] + http['path'].replace('$id$', initial_contact['id']).replace('$siteId$', str(site_id)), http['method'],
                                        None)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_delete_contact_by_id_oauth_token(self, case, http, expect, login, every_function_create_contact):
        login_info = login
        initial_contact = every_function_create_contact
        site_id = ManageGlobalData().get_site_id()
