...
class TestPostContact:
    ...
    def test_post_contact_by_id(self, case, http, expect, login):
        ...
        get_response = test_api_request(login_info['apiUrl'] + http['path'].replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'],change_to_random_int(http['body']))
        commonResponseAssert(get_response, expect)
        afterCaseActionAndAssert(login_info['apiUrl'], login_info['commonHeader'], get_response, expect)
    ...
    def test_post_contact_api_key(self, case, http, expect, login):
        ...
        get_response = test_api_basic_auth_request(
            login_info['apiUrl'] + http['path'].replace('$siteId$',str(site_id)),
            http['method'], change_to_random_int(http['body']))
        commonResponseAssert(get_response, expect)
        afterCaseActionAndAssert(login_info['apiUrl'], login_info['commonHeader'], get_response, expect)
    ...
    def test_post_contact_oauth_token(self, case, http, expect, login):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['apiUrl'] + http['path'].replace('$siteId$', str(site_id)), http['method'], login_info['oauth_header'], change_to_random_int(http['body']))
        commonResponseAssert(get_response, expect)
        afterCaseActionAndAssert(login_info['apiUrl'], login_info['commonHeader'], get_response, expect)
