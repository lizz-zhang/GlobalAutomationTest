...
@allure.feature('get contacts')
class TestGetContacts:
    @pytest.mark.parametrize('case, http, expect', list(parameters), ids=cases)
    @pytest.mark.contactapi
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_contacts(self, case, http, expect, login, create_and_delete_contact):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['apiUrl'] + http['path'].replace('$siteId$', str(site_id)), http['method'], login_info['commonHeader'], None)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_contacts_api_key(self, case, http, expect, login, create_and_delete_contact):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_basic_auth_request(login_info['apiUrl'] + http['path'].replace('$siteId$', str(site_id)), http['method'], None)
        commonResponseAssert(get_response, expect)
    ...
    @pytest.mark.mila
    @pytest.mark.smoke
    def test_get_contacts_oauth_token(self, case, http, expect, login, create_and_delete_contact):
        login_info = login
        site_id = ManageGlobalData().get_site_id()
        get_response = test_api_request(login_info['apiUrl'] + http['path'].replace('$siteId$', str(site_id)), http['method'], login_info['oauth_header'], None)
        commonResponseAssert(get_response, expect)
